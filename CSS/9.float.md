...
Q+A.마지막 4번 방법에서 들었던 의문점이 있는데, 참고했던 블로그에서 inline-block이 float을 해제하는 방법으로 부모 요소에 ```display: inline-block;``` 속성을 부여하는 방법을 제시했다. ```inline-block``` 속성이 부여된 요소는 ```float```된 자식의 높이만큼 늘어나며 ```float```을 해제하는 작용을 한다고 한다. 실제로 적용해보니, 높이는 늘어났지만, 예상했던 레이아웃이 아니었다.

![parent-inline-block](../images/parent-inline-block.png)

[검색해본 결과](http://blog.hivelab.co.kr/2%ED%8C%80-hcrp-%EC%9C%A0%EB%8F%99%EC%A0%81%EC%9D%B8-blockdiv%EC%9A%94%EC%86%8C-%EA%B0%80%EC%9A%B4%EB%8D%B0-%EC%A0%95%EB%A0%AC%ED%95%98%EA%B8%B0/) 네비게이션에서 이런 방식으로 정렬하기도 하나보다.
```html
<html>
<head>
  <style>
