...
마지막 4번 방법에서 들었던 의문점이 있는데, 참고했던 블로그에서 inline-block이 float을 해제하는 방법으로 부모 요소에 ```display: inline-block;``` 속성을 부여하는 방법을 제시했다. ```inline-block``` 속성이 부여된 요소는 ```float```된 자식의 높이만큼 늘어나며 ```float```을 해제하는 작용을 한다고 한다. 실제로 적용해보니, 높이는 늘어났지만, 예상했던 레이아웃이 아니었다.
