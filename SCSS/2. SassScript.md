...
`&`가 부모 선택자를 대체할 뿐 아니라 컴파일된 compile 부모 선택자에 의해 대체될 수도 있다고 생각해보자.  
이것은 2단계 이상의 중첩시에 `&`가 있는 경우 매우 중요한 문제이다. 다음 두 가지 다소 엉뚱한 예시가 이해를 도와줄 것이다.

미리 말하지만, 아래처럼 셀렉터를 사용하지 말라 :)
```scss
//first example: scss
...
`div` 다음으로 오는 두 개의 `&`는 각각 컴파일된 부모 선택자로 대체될 것이다. 따라서 매번 `&` .parent .child를 삽입한다. 다음은 컴파일된 css이다.
...
이 css를 머리속에서 컴파일하면, 최상위 레이어부터 시작해서 바깥 레이어를 벗겨내고 `&`를 새로 컴파일된 부모 셀렉터로 바꾼다.

```css
/*compiles to css*/
.grand-chile .parent .child.sibling {}
```
