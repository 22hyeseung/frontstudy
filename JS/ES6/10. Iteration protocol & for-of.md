...
![이터레이션 프로토콜(Iteration protocol)](http://poiemaweb.com/img/iteration-protocol.png)

심볼은 절대 바뀔 수 없는 것으로, 참조가 불가하다. 그 어떤 프로퍼티 이름과도 중복될 수 없는 아주 고유한 객체이다. 이터레이터는 심볼 타입의 메서드를 가지고 있다.
ES6에서 제공하는 빌트인 이터러블은 아래와 같다. 아래의 객체들은 이터레이션 프로토콜을 준수하고 있으며 이터러이터를 반환한다.  

순회가 완료되었을 때, 이터레이터의 done이 true가 된다.  
