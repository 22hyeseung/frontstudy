...
### 5) ```__prototype__```프로퍼티
__prototype__프로퍼티는 부모 객체를 가리키고 있는다.

### 6) prototype 프로퍼티
<!-- 수 객체는 일반 객체와 다르게 prototype프로퍼티가 존재한다.
생성자 함수는 객체를 생산해 낼 수 있다. 즉, 부모를 가지고 있을 뿐 아니라 부모가 되기도 한다. 일반 객체는 객체를 생산하지 않는다. 따라서 부모가 될 수 없다. prototype프로퍼티는  -->
"prototype":{
  constructor: {}
  __proto: {}
} 
...
ECMAScript spec에서는 **모든 객체는 자신의 프로토타입을 가리키는 [[Prototype]]이라는 숨겨진 프로퍼티를 가진다** (=부모가 있다) 라고 되어있다. (부모를 찾기 위해서는 프로토타입 프로퍼티를 이용해야 한다.)
